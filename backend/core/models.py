...
class CoachProfile(TimeStampedModel):
    ...
    club_name = models.CharField(max_length=150, default=PROJECT_CLUB_NAME)
...
class Match(TimeStampedModel):
    ...
    def result_label(self):
        ...
        club_is_away = self.away_team.casefold() == PROJECT_CLUB_NAME.casefold()
        if self.home_score == self.away_score:
            return "Durang"
        won = self.away_score > self.home_score if club_is_away else self.home_score > self.away_score
        if won:
            return "G'alaba"
        return "Mag'lubiyat"
